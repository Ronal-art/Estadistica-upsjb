...
```{r}
data_sm <- import(here("bac_aguda.csv"))
```
...
```{r}
data_sm |>
  select(numero_caso , año , raza ,sexo , leucocitos_sangre 
         ,neutrofilos_sangre_pct , células_banda_sangre_pct ,glucosa_sangre ,glucosa_lcr 
    ) |>
  ggmice::plot_pattern(
    square = TRUE,
    rotate = TRUE)
...
```{r}
tabla_numero_caso = data_sm |> 
  dplyr::select(numero_caso , año , raza ,sexo , leucocitos_sangre , neutrofilos_sangre_pct , células_banda_sangre_pct ,glucosa_sangre ,glucosa_lcr) |>
...
tabla_leucocitos_sangre = data_sm |> 
  dplyr::select(numero_caso , año , raza ,sexo , leucocitos_sangre ,neutrofilos_sangre_pct , células_banda_sangre_pct ,glucosa_sangre ,glucosa_lcr) |>
  mutate(missing = factor(
    is.na(leucocitos_sangre),
    levels = c(FALSE, TRUE),
    labels = c("Sin valores perdidos", "Con valores perdidos")
  )) |> 
  tbl_summary(
...
tabla <- tbl_merge(
  tbls = list(tabla_numero_caso, tabla_leucocitos_sangre),
  tab_spanner = c("**numero_caso**", "**leucocitos_sangre**")
)
```
...
input_data =
  data_sm |>
    dplyr::select(numero_caso , año , raza ,sexo , leucocitos_sangre , neutrofilos_sangre_pct , células_banda_sangre_pct ,glucosa_sangre ,glucosa_lcr) |> 
  mutate(numero_caso = as.factor(leucocitos_sangre))
```
...
```{r}
ggmice(data_imputada, aes(x = numero_caso, y = glucosa_sangre)) +
  geom_jitter(height = 0, width = 0.25) +
  geom_boxplot(width = 0.5, size = 1, alpha = 0.55, outlier.shape = NA) +
  labs(x = "Imputation number")
```

Para la variables neutrofilos_sangre_pct

```{r}
ggmice(data_imputada, aes(x = numero_caso, y = neutrofilos_sangre_pct)) +
  geom_jitter(height = 0, width = 0.25) +
  geom_boxplot(width = 0.5, size = 1, alpha = 0.55, outlier.shape = NA) +
  labs(x = "Imputation number")
```
...
prop.table(table(data_imputada_l$glucosa_sangre,
...
```{r}
data_glucosa_sangre <- factor(data_imputada$glucosa_sangre, levels = c(0, 1))

```

```{r}
str(data_imputada$glucosa_sangre)
table(data_imputada$glucosa_sangre)
...
tabla_multi <-
  data_imputada |> 
  with(glm(numero_caso  ~ año  + raza  +sexo + leucocitos_sangre  + neutrofilos_sangre_pct  + células_banda_sangre_pct  ,
  ...
  tbl_regression(exponentiate = TRUE,
                 label = list(
                   ...
                   leucocitos_sangre ~ "milímetros cúbicos (mm³)",
                   neutrofilos_sangre_pct ~ "porcentaje (%)",
                   células_banda_sangre_pct  ~ "porcentaje (%)")) |>
  bold_p(t = 0.05) |>
...
numero_caso , año , raza ,sexo , leucocitos_sangre , neutrofilos_sangre_pct , células_banda_sangre_pct ,glucosa_sangre ,glucosa_lcr
