...
```{r}
bac_aguda_1 <- import(here("bac_aguda_1"))
```
...
Antes de realizar este análisis, es necesario definir las categorías de referencia para las variables categóricas mediante la función `mutate()` en combinación con `relevel()`. Este paso asegura que la interpretación de los OR se haga en relación con la categoría de referencia seleccionada. El resultado se guarda en un nuevo objeto llamado

```{r}
bac_aguda_2 <- bac_aguda_1 |> 
  mutate(hipert = relevel(as.factor(mes), ref = "Enero"), edades = relevel(as.factor(año), ref = "78"),
         desenlace = relevel(as.factor(conjunto), ref = "Prueba")
         ) |> 
  na.omit()
```
...
```{r}
tabla_reg_log_univ <- bac_aguda_2 |>
  tbl_uvregression(
    include = c(numero_caso, año
, edad, raza, leucocitos_sangre,neutrofilos_sangre_pct, cultivo_sangre),
    y = desenlace,
    method = glm,
    ...
    label = list(
      numero_caso ~ "nuemero de px",
      año ~ "año de nacimiento",
      edad ~ "variable de edad",
      raza ~ "Negro",
      leucocitos_sangre ~ "leucocitos/mm³",
      neutrofilos_sangre_pct ~ "neutrofilos/mm³",
      cultivo_sangre ~ "cantidad")
  ) |>
  bold_labels() |>
  bold_p(t = 0.05) |>
...
```{r}
var_modelo = glm(
  desenlace ~ numero_caso  + año  + edad + raza  + leucocitos_sangre + 
    neutrofilos_sangre_pct  + cultivo_sangre,
  data = bac_aguda_2,
...
```{r}
tabla_multi <- glm(
  desenlace ~ numero_caso  + año  + edad + leucocitos_sangre ,
  family = binomial(link = "logit"),
  data = bac_aguda_2
) |>
  tbl_regression(
